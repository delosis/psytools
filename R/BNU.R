# Additional functions for the BNU studies Copyright (C) 2017-2019 Delosis This program
...
deriveBnuPDS <- function(df) {
  ...
  df$PDS_sum[df[, grepl("gender", names(df))] == "1"] <-
    rowSumsCustomMissing(df[df[,
                               grepl("gender", names(df))] == "1", grepl("2|4|5", colnames(df))], maxMissing = 1,
  ...
  df$PDS_stage[df[, grepl("gender", names(df))] == "1" &
                 df$PDS_sum >= 12] <- 5
  df$PDS_stage[df[, grepl("gender", names(df))] == "1" &
                 df$PDS_sum >= 9 & df$PDS_sum <=
                 11] <- 4
  df$PDS_stage[df[, grepl("gender", names(df))] == "1" &
                 df$PDS_sum >= 6 & df$PDS_sum <=
                 8] <- 3
  df$PDS_stage[df[, grepl("gender", names(df))] == "1" &
                 df$PDS_sum >= 4 & df$PDS_sum <=
                 5] <- 2
  df$PDS_stage[df[, grepl("gender", names(df))] == "1" &
                 df$PDS_sum < 4] <- 1
...
deriveBnuSC <- function(df) {
  ...
  df$acceptance_and_support <-
    rowMeansCustomMissing(df[, grepl("02|06|10|14|18|22R|26|30|34",
                                     colnames(df))])
  df$fairness_and_justice <-
    rowMeansCustomMissing(df[, grepl("03|07|11|15|19|23|27|31|35",
                                     colnames(df))])
  df$encouragement_and_cooperation <-
    rowMeansCustomMissing(df[, grepl("04|08|12|16|20|24|28|32|36",
                                     colnames(df))])
  df$school_beliefs <-
...
deriveBnuMS <- function(df) {
  ...
  df$T1_MS_Growth <-
    rowSumsCustomMissing(df[, grepl("02|03|05|06|09|10|13|15|18|19",
                                    colnames(df))])
  df$T1_MS_Fixed <-
    rowSumsCustomMissing(df[, grepl("01|04|07|08|11|12|14|16|17|20",
                                    colnames(df))])
  df$T1_MS_4 <-
    ifelse(
      df$T1_MS_Growth + 30 - df$T1_MS_Fixed < 21,
      1,
      ifelse(
        df$T1_MS_Growth +
          30 - df$T1_MS_Fixed < 34,
        2,
        ifelse(df$T1_MS_Growth + 30 - df$T1_MS_Fixed < 45,
               3, 4)
      )
    )
  df$T1_MS_2 <-
...
deriveBnuBG <- function(df) {
  ...
  if ("T1_BG_PC3" %in% names(df)) {
    df$Child_Age_Days <-
      floor(difftime(df$Processed.Timestamp, df$T1_BG_PC3, "days"))
  }
...
deriveBnuSPSRQ <- function(df) {
  ...
  df$Impulsivity_Fun_Seeking <-
    rowSumsCustomMissing(df[, grepl("11|14|20|22|24|25|32",
                                    colnames(df))])
  ...
  df$Reward_Sensitivity <-
    rowSumsCustomMissing(df[, grepl("Impulsivity_Fun_Seeking|Reward_Responsivity|Drive_Scale",
                                    colnames(df))])
